    ...
    <section className="bg-white/60 py-12">
      <div className="mx-auto grid w-full max-w-6xl gap-4 px-5 sm:grid-cols-2 sm:px-6 lg:grid-cols-4 lg:px-8">
        {trustItems.map((item) => {
          ...
          return (
            <div key={item.title} className="rounded-3xl border border-mist bg-white p-5 shadow-sm">
              <div className="grid h-12 w-12 place-items-center rounded-2xl bg-mist text-brand">
                <Icon className="h-6 w-6" aria-hidden="true" />
              </div>
              <p className="mt-4 text-base font-black leading-snug text-textMain">{item.title}</p>
            </div>
          );
        })}
      </div>
