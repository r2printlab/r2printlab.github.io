...
export function FeaturedMonster() {
  const [imageFailed, setImageFailed] = useState(false);
  const imageSrc = imageFailed ? monsterProduct.fallbackImage : monsterProduct.image;
  const productIsActive = monsterProduct.productUrl !== "#";
  const whatsappIsActive = monsterProduct.whatsappUrl !== "#";
    ...
    <section className="mx-auto w-full max-w-6xl px-5 py-12 sm:px-6 lg:px-8">
      <div className="grid items-center gap-8 rounded-[2rem] border border-mist bg-white p-5 shadow-soft dark:border-darkAccent/20 dark:bg-darkCard sm:p-8 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <div>
          ...
          <h2 className="mt-3 text-3xl font-black leading-tight text-textMain dark:text-darkText sm:text-4xl">{monsterProduct.title}</h2>
